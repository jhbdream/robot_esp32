...
/*
*使用GPIO
*  mpu6050 SCL-> IO18  SDA->IO19
*  ADC GPIO32   GPIO33   GPIO34   GPIO35
*
*
*
*
...
{	   
    ...
    while (1) {
        ...
        //sprintf(data,"P%4.0fR%4.0f",Pitch,Roll);
       ...
       // printf("Pitch is: %d,Roll is %d,Yaw is %d\n",C_Pitch,C_Roll,C_Yaw);
        ...
        printf("%4d %4d %4d %4d %4d %4d %4d\n",C_Pitch,C_Roll,C_Yaw,adc.read[0],adc.read[1],adc.read[2],adc.read[3]);

        uxBits = xEventGroupWaitBits(wifi_event_group, MQTT_CONNECTED_BIT , false, false, 0); 
        if( uxBits & MQTT_CONNECTED_BIT )
          { 
            //mqtt_publish(Myclient, T_HEAD, (char *)data,11, 0,  0);  
            mqtt_publish(Myclient, T_HANDL, (char *)data,11, 0, 0);  
            //mqtt_publish(Myclient, T_HANDR, (char *)data,11, 0, 0);  
          }
